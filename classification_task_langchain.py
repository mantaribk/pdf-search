...
class NumberResult(BaseModel):
    ...
    formatted_value: int = Field(
        description="only number without other symbols and units"
    )
    semantic_label: str = Field(
        ..., description="One of predefined labels (e.g., grant_amount)"
    )
    semantic_category: str = Field(
        ..., description="One of predefined categories (e.g., FINANCIAL VALUES)"
    )
